...
function isAuthenticated(req, res, next) {
  if (req.session && req.session.pin) {
    next();
  } else {
...
app.get('/api/member-info', isAuthenticated, async (req, res) => {
  const pin = req.session.pin;
  // Fetch member info, dependents, etc. from DB
  // Return as JSON
});
...
app.post('/api/change-password', isAuthenticated, async (req, res) => {
  ...
  // 2. Compare oldPassword with hash
  // 3. If match, hash newPassword and update DB
...
fetch('/api/member-info')
  .then(res => res.json())
  .then(data => {
    // Populate dashboard fields with data
  });
